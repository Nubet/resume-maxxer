...
#let contact-text(data, settings) = block(width: 100%)[
  #let profiles = (
    if "email" in data.personal.contact and data.personal.contact.email != none { box(link("mailto:" + data.personal.contact.email, data.personal.contact.email)) },
    if ("phone" in data.personal.contact and data.personal.contact.phone != none) { box(link("tel:" + data.personal.contact.phone, data.personal.contact.phone)) } else { none },
