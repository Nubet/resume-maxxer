...
#let font = ("Arial", "Segoe UI", "Calibri", "Tahoma", "Verdana")
...
#let section-marker(fill) = box(width: 10pt, height: 10pt, radius: 2pt, fill: fill)
...
#let section(
  ...
  context {
    ...
    text(fill: c)[== #section-marker(c) #h(8pt) #name]
    pad(y: 10pt)[#content]
  }
}
...
#let contact = (
  ...
  contact-row("Website", website)
  contact-row("X", x)
}

#let render-area(text-fill, content) = {
  pad(y: 12pt, left: 14pt, right: 10pt, [
    #set text(
      font: font,
      fill: text-fill,
      size: 10.5pt,
