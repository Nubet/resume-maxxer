...
export const AiModal: React.FC<AiModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleApplyExperience = () => {
    ...
    const newHighlights = improvedExp.split("\n").map(line => line.replace(/^•\s*/, "").trim()).filter(Boolean);
    updateResumeData((prev: any) => {
  ...
  const handleAnalyzeJob = () => {
    ...
    setIsProcessing(true);
    setTimeout(() => {
      const userSkills = resumeData?.skills?.flatMap(group => group.keywords.map(k => k.toLowerCase())) || [];
      const commonTech = ["react", "typescript", "node.js", "docker", "aws", "graphql", "sql", "git", "scrum", "rest api", "ci/cd", "agile", "next.js", "python", "kubernetes", "fastapi", "rust"];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
      <div className="relative w-full max-w-2xl overflow-hidden rounded-3xl border border-border bg-surface shadow-2xl text-content flex flex-col max-h-[90vh] animate-scale-in">
        <div className="flex items-center justify-between border-b border-border bg-surface-tertiary p-5 flex-shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-content-secondary">Inteligentna pomoc i analiza słów kluczowych</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex overflow-x-auto border-b border-border bg-surface-secondary px-5 flex-shrink-0 scrollbar-none">
          <button
          ...
          {activeTab === "summary" && (
            <div className="space-y-4 animate-fade-in">
              ...
              >
                ...
                <span>{isProcessing ? "Generowanie..." : "Generuj profesjonalne podsumowanie"}</span>
              </button>
            </div>
          )}
          ...
          {activeTab === "experience" && (
            <div className="space-y-4 animate-fade-in">
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 rounded-full bg-content py-3.5 text-xs font-bold text-content-inverse hover:bg-neutral-800 disabled:opacity-50 transition-all shadow-md active:scale-[0.98]"
              >
                {isProcessing ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4 text-amber-400" />}
                <span>{isProcessing ? "Optymalizowanie..." : "Przekształć w mocne osiągnięcia (Action Verbs)"}</span>
              </button>
            </div>
          )}
          ...
          {activeTab === "tailor" && (
            <div className="space-y-4 animate-fade-in">
              ...
              {gapReport && (
                <div className="rounded-3xl border border-border bg-surface-secondary p-5 space-y-5 animate-fade-in">
                  <div className="flex items-center justify-between border-b border-border pb-3.5">
                    <div>
                      <h4 className="text-sm font-bold text-content">Dopasowanie Słów Kluczowych</h4>
                      <p className="text-xs text-content-secondary">Analiza porównawcza z ofertą pracy</p>
                    </div>
                    <span className={`text-lg font-black px-3.5 py-1 rounded-full border ${
                      gapReport.match_percentage >= 80 ? "bg-surface-tertiary text-content border-content" : "bg-surface-tertiary text-content-secondary border-border"
                    }`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="rounded-2xl bg-surface border border-border p-4 space-y-2.5 shadow-sm">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {gapReport.present_keywords.length > 0 ? gapReport.present_keywords.map((kw, i) => (
                          <span key={i} className="rounded-lg bg-surface-tertiary border border-border px-2.5 py-1 text-[11px] text-content font-semibold">
                            {kw}
                          </span>
                        )) : (
                          <span className="text-content-muted text-[11px]">Brak dopasowań bezpośrednich</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="rounded-2xl bg-surface border border-border p-4 space-y-2.5 shadow-sm">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {gapReport.missing_keywords.length > 0 ? gapReport.missing_keywords.map((kw, i) => (
                          <span key={i} className="rounded-lg bg-surface-secondary border border-border-strong px-2.5 py-1 text-[11px] text-content-secondary font-bold">
                            + {kw}
                          </span>
                        )) : (
                          <span className="text-content-muted text-[11px]">Brak! Masz wszystkie słowa.</span>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 rounded-full bg-content py-3.5 text-xs font-bold text-content-inverse hover:bg-neutral-800 disabled:opacity-50 transition-all shadow-md active:scale-[0.98]"
              >
                {isProcessing ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Lightbulb className="h-4 w-4 text-amber-400" />}
                <span>{isProcessing ? "Analizowanie ogłoszenia..." : "Przeanalizuj luki w słowach kluczowych"}</span>
              </button>
            </div>
          )}
        </div>
