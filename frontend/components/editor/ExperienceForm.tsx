      ...
      {experience.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-border p-12 text-center space-y-3 bg-surface-secondary">
          ...
          <p className="text-xs text-content-muted max-w-xs mx-auto leading-relaxed">
            Dodaj swoje miejsca pracy, staże lub projekty komercyjne. Pamiętaj o używaniu liczb i języka korzyści!
          </p>
        </div>
      ) : (
          ...
          {experience.map((exp, index) => (
            <div key={index} className="rounded-3xl border border-border bg-surface-secondary p-8 space-y-6 transition-all hover:border-border-strong">
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                  <label className="block text-xs font-bold text-content mb-2">Stanowisko / Rola</label>
                  <input
                    type="text"
                    value={exp.position || ""}
                ...
                <div>
                  <label className="block text-xs font-bold text-content mb-2">Nazwa Firmy / Organizacji</label>
                  <input
                    type="text"
                    value={exp.company || ""}
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-content mb-2">Lokalizacja (Miasto / Tryb)</label>
                  <input
                    type="text"
                    value={exp.location || ""}
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-content mb-2">Opis Obowiązków i Osiągnięć (każda linia to nowy punkt)</label>
                  <p className="text-[11px] text-content-secondary mb-2 leading-relaxed">
                    Opisz swoje kluczowe sukcesy. Każda nowa linia zostanie zmapowana jako punktor w kompilatorze Typst.
                  </p>
                  <textarea
                    rows={5}
