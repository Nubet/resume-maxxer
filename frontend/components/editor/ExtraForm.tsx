      ...
      <div className="space-y-6">
        ...
        {languages.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-border p-8 text-center space-y-2 bg-surface-secondary">
            ...
            <p className="text-[11px] text-content-muted">Dodaj języki obce oraz poziom biegłości (np. Angielski - C1 / Biegły).</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {languages.map((lang, idx) => (
              <div key={idx} className="rounded-2xl border border-border bg-surface-secondary p-5 flex items-center justify-between gap-4">
      ...
      <div className="space-y-6 pt-6 border-t border-border">
        <div className="flex items-center justify-between border-b border-border pb-5">
          <div className="flex items-center space-x-2.5">
            ...
            <h3 className="text-xl font-bold tracking-tight text-content">Certyfikaty, Licencje i Nagrody</h3>
          </div>
          <button
        ...
        {certifications.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-border p-8 text-center space-y-2 bg-surface-secondary">
            ...
            <p className="text-[11px] text-content-muted">Dodaj zdobyte certyfikaty branżowe (np. AWS Certified Solutions Architect, Scrum Master).</p>
          </div>
        ) : (
          <div className="space-y-4">
            {certifications.map((cert, idx) => (
              <div key={idx} className="rounded-3xl border border-border bg-surface-secondary p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <div className="sm:col-span-2">
                    <label className="block text-[11px] font-bold text-content mb-1">Nazwa Certyfikatu / Nagrody</label>
                    <input
                      type="text"
                      value={cert.name || ""}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-content mb-1">Data Uzyskania (YYYY-MM)</label>
                    <input
                      type="text"
                      value={cert.date || ""}
                  ...
                  <div className="sm:col-span-3">
                    <label className="block text-[11px] font-bold text-content mb-1">Wystawca / Organizacja</label>
                    <input
                      type="text"
                      value={cert.issuer || ""}
